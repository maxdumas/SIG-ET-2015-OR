	...
	float Show( bool show, bool immediate = false ) {
		...
		if ( show ) {
			...
			if ( currentCamera != null ) {
				Vector3 offset = ( currentCamera.transform.forward * distanceFromViewer );
				offset.y = ( transform.position.y - currentCamera.transform.position.y );
				transform.position = currentCamera.transform.position + offset;
				Vector3 dirToCamera = ( currentCamera.transform.position  - transform.position );
				dirToCamera.y = 0.0f;
